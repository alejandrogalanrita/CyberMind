...
async function loadAllProjects() {
  try {
    const response = await fetch(API_ROUTE + "/get_all_projects", {
      method: "GET",
      credentials: 'include',
    });
...
async function loadAllDeletedProjects() {
  try {
    const response = await fetch(API_ROUTE + "/get_all_deleted_projects", {
      method: "GET",
      credentials: 'include',
    });
    if (response.status === 401) {
...
async function loadAllUsers() {
  try {
    const response = await fetch(API_ROUTE + "/get_all_users", {
      method: "GET",
      credentials: 'include',
    });
    if (response.status === 401) {
...
document.getElementById("registerForm").addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch(API_ROUTE+ "/register_user", {
        method: "POST",
        body: formData,
        credentials: 'include',
      });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  deleteButton.addEventListener("click", async function (e) {
    ...
    for (let checkbox of selectedCheckboxes) {
      ...
      try {
        const response = await fetch(API_ROUTE + "/delete_user", {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
...
document.getElementById("editForm").addEventListener("submit", async function (e) {
  ...
  const response = await fetch(API_ROUTE+ "/edit_user", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
